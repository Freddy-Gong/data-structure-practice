...
var addChildren = function addChildren(node, value) {
  var newNode = {
    data: value,
    children: null,
    parent: node
...
var find = function find(tree, node) {
  if (tree === node) {
    ...
    for (var i = 0; i < tree.length; i++) {
      var result = find(tree.children, node);
...
var removeNode = function removeNode(tree, node) {
  var sibling = node.parent.children; //这里不用便利，因为node里面又parent的属性，能够直接关联到tree，所以不用遍历tree。
...
var node2 = addChildren(tree, 20);
var node3 = addChildren(tree, 30);
addChildren(tree, 40);
